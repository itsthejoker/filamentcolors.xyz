if (!customElements.get('fc-colormatch')) {
  class ColorMatch extends HTMLElement {
    ...
    _onLoad() {
      ...
      // Initial sync
      if (window.picker && (window.inputs['hex'].value === '' || window.inputs['hex'].value === '#45FFC1')) {
        this.updateAll()
      } else if (window.picker) {
        // If there's already a value (e.g. pre-filled by server), sync other fields TO it
        this.changeHexColorPicker()
      }
