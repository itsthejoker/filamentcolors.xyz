...
def hex_to_rgb(in_col):
    ...
    return tuple(
        [int(in_col[s:s + 2], 16) for s in range(0, len(in_col), 2)]
    )
...
class Color:
    ...
    def __init__(self, in_col):
        ...
        assert (
                is_rgb(in_col) or
                is_hex(in_col) or
                in_col.lower() in self.colors.keys()
        ), (
        ...
        if (
                is_rgb(in_col) is False and
                is_hex(in_col) is False and
                in_col.lower() in self.colors.keys()
        ):
    ...
    def next_color(self):
        ...
        if R == 255 and G == 255 and B == 255:
            raise ValueError(
                'R, G and B all have the value 255, no further colors are '
                'available.'
            )
...
def test():
    ...
    assert x.tints() == ['#01f490', '#11fe9d', '#2cfea8', '#46feb3', '#61febd', '#7bfec8', '#95ffd3', '#b0ffde',
                         '#caffe9', '#e5fff4', '#ffffff']
    assert x.shades() == ['#01f490', '#01dc82', '#01c373', '#01ab65', '#019256', '#017a48', '#00623a', '#00492b',
                          '#00311d', '#00180e', '#000000']
    assert x.saturate() == ['#01f490', '#01f490', '#01f490', '#01f490', '#01f490', '#00f490', '#00f590', '#00f590',
                            '#00f590', '#00f590', '#00f590']
    assert x.desaturate() == ['#01f490', '#0de88e', '#19dc8c', '#25d08a', '#32c387', '#3eb785', '#4aab83', '#569f81',
                              '#62937f', '#6e877d', '#7a7a7a']
    ...
    assert x.tints() == [(1, 244, 144), (17, 254, 157), (44, 254, 168), (70, 254, 179), (97, 254, 189), (123, 254, 200),
                         (149, 255, 211), (176, 255, 222), (202, 255, 233), (229, 255, 244), (255, 255, 255)]
    assert x.shades() == [(1, 244, 144), (1, 220, 130), (1, 195, 115), (1, 171, 101), (1, 146, 86), (1, 122, 72),
                          (0, 98, 58), (0, 73, 43), (0, 49, 29), (0, 24, 14), (0, 0, 0)]
    assert x.saturate() == [(1, 244, 144), (1, 244, 144), (1, 244, 144), (1, 244, 144), (1, 244, 144), (0, 244, 144),
                            (0, 245, 144), (0, 245, 144), (0, 245, 144), (0, 245, 144), (0, 245, 144)]
    assert x.desaturate() == [(1, 244, 144), (13, 232, 142), (25, 220, 140), (37, 208, 138), (50, 195, 135),
                              (62, 183, 133), (74, 171, 131), (86, 159, 129), (98, 147, 127), (110, 135, 125),
                              (122, 122, 122)]
