...
def clamp(n, a, b):
    # https://gist.github.com/TedAlden/1a30101620d2152ab84e9e26b80e8384
...
def is_hsl(in_col):
    ...
    if len(in_col) == 3 and type(in_col) == tuple:
        if 0 <= in_col[0] <= 1 and 0 <= in_col[1] <= 1 and 0 <= in_col[2] <= 1:
            ...
            floating_point_error = 1.0001
            if (
                0 <= in_col[0] < floating_point_error
                and 0 <= in_col[1] < floating_point_error
                and 0 <= in_col[2] < floating_point_error
            ):
                return True
            return False
    else:
