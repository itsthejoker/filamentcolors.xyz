...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Ensure the auxiliary SQLite connection is always closed (fixes test ResourceWarning)
        with closing(
            sqlite3.connect(os.path.join(settings.BASE_DIR, "pantone.db"))
        ) as con:
            cur = con.cursor()
            ...
            if Pantone.objects.count() == 0:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        "There are already Pantone objects present. Skipping."
                    )
                )
